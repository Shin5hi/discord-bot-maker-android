...
The Grid branding is documented in `ui/AppTheme.kt` via a descriptive header comment block. The splash screen (`ui/SplashScreen.kt`) features a pulsing "G" logo placeholder on a Blurple gradient background.

---

## Art Direction

**Theme:** Discord Design Language (Dark) with Grid branding overlay
...
### Typography
- **Font family:** System Sans-Serif (`FontFamily.SansSerif`) across all screens — Inter / Roboto feel.
...
### Grid Branding Integration (2026-05-03)
- **Product rename:** "Discord Bot Maker" → "Grid Bot Hub" in all user-facing UI.
- **Splash screen:** New `ui/SplashScreen.kt` — Blurple gradient background, pulsing geometric "G" logo, auto-navigates to dashboard.
- **Dashboard header:** Replaced generic "Dashboard" title with Grid brand mark (Blurple "G" badge) + "Grid Bot Hub" text.
- **Footer:** Version string updated to "Grid Bot Hub v1.0.0".
- **Navigation:** `SplashScreen` added as `startDestination` in `AppNavGraph`; pops itself after 2s delay.
- **AppTheme.kt:** Added Grid branding identity comment block documenting logo, colors, and typeface direction.
- **Typography comment:** Updated to reference Inter/Roboto professional direction.
...
- ~~Generic "Dashboard" header title — replaced with "Grid Bot Hub"~~
...
| File | Owner | Status |
|------|-------|--------|
| `ui/AppTheme.kt` | UI | ✅ Active — Grid branding + Discord palette |
| `ui/SplashScreen.kt` | UI | ✅ Active — Grid branded splash |
| `ui/MainDashboardScreen.kt` | UI | ✅ Active — Grid Bot Hub header |
| `ui/AppNavigation.kt` | UI | ✅ Active — Splash → Dashboard flow |
